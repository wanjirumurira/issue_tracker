...
def create_project(request):
    ...
    if request.method == 'POST':
        ...
        if project_form.is_valid():
            project_form.save()
            messages.info(request, "Project successfully created")
...
def updateProject(request, pk):
    ...
    if request.method == 'POST':
        ...
        if project_update.is_valid():
            project_update.save()
            messages.info(request, "Project successfully updated")
            return redirect('projects')
...
def create_issues(request, pk):
    ...
    if request.method == 'POST':
        ...
        if issue_form.is_valid():
            issue_form.save()
            messages.info(request, "Issue successfully created")
    ...
    if request.method == 'POST':
        ...
        if password == password2:
            if User.objects.filter(username=username).exists():
                ...
                user = User.objects.create(username=username, email=user_email, password=password)
